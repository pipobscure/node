...
function finishVfsMounts() {
  ...
  const loadEntry = getOptionValue('--vfs-load');
  const loadIndex = getOptionValue('--vfs-load-index');
  for (let i = 0; i < entries.length; i++) {
    ...
    // --vfs-load runs the mount its 0-based index selects. That mount's
    // reserved mount point is what the entry is require()d from; process.argv[1]
    // names the real source instead, since the reserved point is an opaque
...
function patchProcessObject(expandArgv1) {
  ...
  if (expandArgv1 && !getOptionValue('--vfs-load') &&
