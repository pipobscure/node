...
function prepareExecution(options) {
  ...
  // Needed before finishVfsMounts() below: mounting a VFS logs through
  // internal/util/debuglog, which throws if used before this runs.
  setupDebugEnv();

  // If --vfs-mount is active, resolve its targets now - with --vfs-load the
  // main entry point below anchors on the last mount's root rather than the
  // real cwd - but defer choosing providers and mounting until
  // finishVfsMounts() below, after -r preload modules have had a chance to
  // register a custom provider for a target.
  const vfsMounts = resolveVfsMounts();

  // Patch the process object and get the resolved main entry point.
  const mainEntry = patchProcessObject(expandArgv1, vfsMounts.loadTarget);
  ...
  // With no --import, all preloads (-r) have run on the main thread, so mount
  // the --vfs-mount targets now. Otherwise defer until after --import runs:
  // finishVfsMounts() is called again (idempotently) from
  // asyncRunEntryPointWithESMLoader in internal/modules/run_main once -r and
  // --import preloads are both done - which also covers worker threads, whose
  // preloads all run later. A preload may register a custom provider first.
...
// The --vfs-mount targets resolved by resolveVfsMounts(), pending until
// finishVfsMounts() mounts them (exactly once, guarded by vfsMountsDone).
let pendingVfsMounts = [];
let vfsMountsDone = false;

/**
 * Resolves and validates every --vfs-mount entry without yet choosing a
 * provider or mounting, storing the result for finishVfsMounts(). Each entry
 * is `<source>` or `<source>=<target>`, where `<source>` is the directory or
 * archive to mount and `<target>` (optional) is where to mount it, defaulting
 * to `<source>`'s own resolved path. Kept separate from finishVfsMounts() so
 * the --vfs-load entry point's path can anchor process.argv[1] resolution
 * early (that resolution anchors on the mount's root rather than the real
 * cwd), while provider selection is deferred until after -r/--import preload
 * modules have run - see prepareExecution().
 * @returns {{ loadTarget: string | undefined }} The mount point to run as the
 *   entry point when --vfs-load is set, otherwise undefined.
 */
function resolveVfsMounts() {
  const entries = getOptionValue('--vfs-mount');
  if (entries.length === 0) return { loadTarget: undefined };
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    ArrayPrototypePush(mounts, { source: resolvedSource, mountPoint, stats });
  }
  pendingVfsMounts = mounts;

  // --vfs-load runs the entry point from the last --vfs-mount. (The C++ option
  // parser already rejects --vfs-load without any --vfs-mount.)
  const loadTarget = getOptionValue('--vfs-load') ?
    mounts[mounts.length - 1].mountPoint : undefined;
  return { loadTarget };
}

/**
 * Chooses a provider for each resolved --vfs-mount (see resolveVfsMounts())
 * and mounts it. Module resolution then finds the mounts the same way any
 * other VFS mount is found, via internal/vfs/setup.js's activeVFSList. Runs
 * after -r and --import preload modules, so a preloaded module can register
 * (through node:vfs's registerProvider()) a custom provider; custom providers
 * are always tried before the built-ins. A file with no matching provider is
 * rejected. Idempotent: called both from prepareExecution() (main thread, no
 * --import) and from asyncRunEntryPointWithESMLoader() in
 * internal/modules/run_main (after --import); only the first call mounts.
 */
function finishVfsMounts() {
  if (vfsMountsDone || pendingVfsMounts.length === 0) return;
  vfsMountsDone = true;
  const { VirtualFileSystem } = require('internal/vfs/file_system');
  const { selectProvider } = require('internal/vfs/provider_registry');
  for (let i = 0; i < pendingVfsMounts.length; i++) {
    const { source, mountPoint, stats } = pendingVfsMounts[i];
    const provider = selectProvider(source, stats);
    if (provider === null) {
      throw new ERR_VFS_INVALID_TARGET(source);
    }
