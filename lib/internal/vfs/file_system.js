...
function randomSuffix() {
  let suffix = '';
  for (let i = 0; i < 6; i++) {
    suffix += kTempChars[(MathRandom() * kTempChars.length) | 0];
  }
  return suffix;
}
...
class VirtualFileSystem {
  ...
  /**
   * Returns true when `normalizedInput` falls inside this VFS's
   * reserved mount point. The caller must have already passed the
   * input through `normalizeMountedPath`; the dispatch loop in
   * `internal/vfs/setup.js` normalizes once and hands the same value
   * to every candidate layer.
  ...
  mkdirSync(dirPath, options) {
    ...
    // A recursive mkdir reports the first directory it created, which the
    // provider names relative to itself.
   ...
   */
  mkdtempSync(prefix) {
    const dirPath = this.#toProviderPrefix(prefix) + randomSuffix();
    this[kProvider].mkdirSync(dirPath);
    return this.#toMountedPath(dirPath);
  }

  /**
   * Converts a mkdtemp prefix to a provider-relative one. The prefix is
   * text that the random suffix is appended to, not a path to resolve: a
   * trailing separator means "inside this directory", and resolving would
   * drop it and turn `dir/` + suffix into a sibling of `dir`.
   * @param {string} prefix The mounted prefix
   * @returns {string}
   */
  #toProviderPrefix(prefix) {
    const last = prefix[prefix.length - 1];
    const trailing = last === '/' || last === sep;
    const providerPrefix = this.#toProviderPath(prefix);
    if (!trailing) return providerPrefix;
    return providerPrefix === '/' ? '/' : `${providerPrefix}/`;
  }
  ...
  #createPromisesAPI() {
    ...
    return ObjectFreeze({
      ...
      async mkdtemp(prefix) {
        const dirPath = toProviderPrefix(prefix) + randomSuffix();
