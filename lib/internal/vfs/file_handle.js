...
class VirtualFileHandle {
  ...
  /**
   * Changes the file mode. Providers whose handles carry no metadata leave
   * this a no-op; those that do override it.
  ...
  /**
   * Changes the timestamps. Providers whose handles carry no metadata leave
   * this a no-op; those that do override it.
...
class MemoryFileHandle extends VirtualFileHandle {
  ...
  /**
   * Changes the permission bits of the underlying entry, as fchmod(2) does
   * through a descriptor. The type bits are kept.
   * @param {number} mode The new permission bits
   */
  chmodSync(mode) {
    this.#checkClosed('fchmod');
    // Validated here as well because a `FileHandle` calls this method
    // directly with the caller's argument.
  ...
  /**
   * Changes the timestamps of the underlying entry, as futimes(2) does
   * through a descriptor. Accepts what `fs.utimes` accepts; the values are
   * validated and converted here because a `FileHandle` calls this method
   * directly with the caller's arguments.
   * @param {Date|number|string} atime The new access time
   * @param {Date|number|string} mtime The new modification time
   */
  utimesSync(atime, mtime) {
