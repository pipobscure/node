  ...
  return {
    ...
    watch(filename, options, listener) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) {
          // A provider watcher polls whatever it is given; the real fs
          // refuses up front when there is nothing to watch.
          if (!r.vfs.existsSync(pathStr)) throw createENOENT('watch', pathStr);
          return r.vfs.watch(pathStr, options, listener);
        }
      }
      return undefined;
    },
    ...
    unwatchFile(filename, listener) {
      ...
      const r = findVFSForPath(pathStr);
      if (r === null) return undefined;
      r.vfs.unwatchFile(pathStr, listener);
      return true;
    },
    promisesWatch(filename, options) {
      ...
      if (r === null) return undefined;
      if (!r.vfs.existsSync(pathStr)) throw createENOENT('watch', pathStr);
      return r.vfs.promises.watch(pathStr, options);
    },
