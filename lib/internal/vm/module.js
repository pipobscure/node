...
class Module {
  ...
  get namespace() {
    ...
    return this[kWrap].getNamespaceSync();
  }
  ...
  evaluate(options = kEmptyObject) {
    ...
    if (this[kWrap].isGraphAsync()) {
      return this[kWrap].evaluate(timeout, breakOnSigint);
    }
    return this[kWrap].evaluateSync(timeout, breakOnSigint);
  }
...
class SourceTextModule extends Module {
  ...
  [kLink](linker) {
    ...
    const moduleResults = Array(moduleRequests.length);
    ...
    for (let idx = 0; idx < moduleRequests.length; idx++) {
      ...
      const postLink = (module) => {
        ...
        if (module.status === 'unlinked') {
          linking = module[kLink](linker);
        }
        if (linking?.then) {
          return linking.then(()=>module[kWrap]);
        }
        return module[kWrap];
      };
      if (linkerResult?.then) {
        asynced += 1;
        moduleResults[idx] = PromisePrototypeThen(linkerResult, postLink);
      } else {
        moduleResults[idx] = postLink(linkerResult);
      }
    ...
    if(asynced > 0) {
      return Promise.PromisePrototypeThen(
        SafePromiseAllReturnArrayLike(moduleResults),
        finalize,
      );
    }
    finalize(moduleResults);
  }
