 ...
 * parent's whole per-isolate options (including --vfs-mount/--vfs-load) via
...
function withInheritedVfsFlags(execArgv) {
  ...
  if (getOptionValue('--vfs-load') &&
      !ArrayPrototypeSome(execArgv, (arg) => StringPrototypeStartsWith(arg, '--vfs-load'))) {
    ArrayPrototypePush(toInject, '--vfs-load');
  }
