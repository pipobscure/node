...
async function asyncRunEntryPointWithESMLoader(callback) {
  ...
  try {
    ...
    // Now that -r and --import preloads have all run, mount any --vfs-mount
    // targets so a provider registered by either backs them. Idempotent: a
    // no-op when the main thread already mounted in prepareExecution() (no
    // --import present).
...
function executeUserEntryPoint(main = process.argv[1]) {
  ...
  // With --vfs-load the entry point is the mount root, a directory, which only
  // resolves to its index.js / package.json "main" once the mount is active.
  // When --import or a custom loader forces the ESM loader, the mount is not
  // active until finishVfsMounts() runs inside runEntryPointWithESMLoader
  // (after --import), which is after resolveMainPath() would normally run - and
  // the ESM loader cannot import a directory. So defer resolving the entry into
  // that flow, where the mount is in place and the directory resolves to a file
  // the ESM loader accepts.
  const deferResolve = !isEntryURL && getOptionValue('--vfs-load') &&
    (getOptionValue('--import').length > 0 ||
     getOptionValue('--experimental-loader').length > 0);

  let useESMLoader;
  let resolvedMain;
  if (isEntryURL || deferResolve) {
    useESMLoader = true;
  } else {
  ...
  if (!useESMLoader) {
    ...
    runEntryPointWithESMLoader((cascadedLoader) => {
      // `deferResolve` postpones resolveMainPath() to here, after --import and
      // the VFS mount are in place (see above).
      const mainPath = deferResolve ? (resolveMainPath(main) || main) : (resolvedMain || main);
      const mainURL = isEntryURL ? new URL(mainPath, getCWDURL()) : pathToFileURL(mainPath);
