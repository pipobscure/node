...
// Exercises the --vfs-mount / --vfs-load startup flags end to end, by actually
// spawning `node --vfs-load --vfs-mount=<fixture>` against small CJS/ESM
// fixture trees and ZIP archives built at test time: entry-point resolution,
// package.json "type"/main-field/node_modules resolution happening entirely
// inside the mount, the mount being scoped to its own target (not the whole
// real filesystem), worker_threads inheriting the mount, and a clear error for
// an invalid target.
//
// With --vfs-load, argv[1] is unconditionally the mount root - as with
...
function runVfs(target, extraArgs = []) {
  return spawnSync(
    process.execPath,
    ['--experimental-vfs', '--vfs-load', `--vfs-mount=${target}`, ...extraArgs],
    { encoding: 'utf8' },
  );
}
...
(async () => {
  ...
  // -- what lets a self-mounting shebang app                              -----
  // -- (`#!/usr/bin/env -S node --vfs-load --vfs-mount`) take its own args. ---
  ...
  // -- --vfs-mount is gated behind --experimental-vfs -------------------------
  //
...

  {
    ...
    assert.notStrictEqual(result.status, 0);
    assert.match(result.stderr, /--vfs-mount requires --experimental-vfs/);
